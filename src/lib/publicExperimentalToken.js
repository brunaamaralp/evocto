const EXPER_PREFIX = 'nave-exp:v1:';
 ...
 * @param {string} secret
 */
export async function createPublicExperimentalToken(academyId, salt, secret) {
  const aid = String(academyId || '').trim();
  const s = String(salt || '').trim();
  if (!aid || !s || !secret) return '';
  const textEnc = new TextEncoder();
  const sig = await hmacSha256(`${EXPER_PREFIX}${aid}:${s}`, secret);
 ...
 */
export async function verifyPublicExperimentalToken(token, secret) {
  ...
  const expected = await hmacSha256(`${EXPER_PREFIX}${academyId}:${salt}`, secret);
