...
export function useTriggerSystem() {
  ...
  // Registrar regras específicas de briefing
  useEffect(() => {
    ...
    registerRule({
      ...
      action: async (event) => {
        if (event.serviceId && event.entityId) {
          console.log(`[TriggerSystem] Aplicando regras de IA para briefing ${event.entityId}`);
          ...
          try {
            ...
            // Aplicar regras de IA
            const adjustments = await aiRulesService.applyRules(briefing);
            
            // Persistir ajustes
            for (const adjustment of adjustments) {
              briefingService.adjustments.set(adjustment.id, adjustment);
            }
            
            console.log(`[TriggerSystem] ${adjustments.length} ajustes aplicados ao briefing ${event.entityId}`);
            toast.success(`${adjustments.length} ajustes de IA aplicados ao briefing`);
            
          } catch (error) {
            console.error(`[TriggerSystem] Erro ao aplicar regras de IA:`, error);
            throw error;
          }
        }
      }
    ...
    // Regra: Briefing Ativado -> Notificar Equipe
    registerRule({
      id: 'briefing_activated_notification',
      eventName: 'briefing_activated',
      description: 'Notifica equipe quando briefing é ativado',
      action: async (event) => {
        if (event.serviceId && event.entityId) {
          console.log(`[TriggerSystem] Notificando equipe sobre briefing ativado ${event.entityId}`);
          ...
          try {
            ...
            // Notificar equipe
            toast.success(`Briefing ativado para serviço ${event.serviceId}`);
            
            // Aqui seria integrado com sistema de notificações
            // await notificationService.notifyTeam({
            //   type: 'briefing_activated',
            //   serviceId: event.serviceId,
            //   briefingId: event.entityId,
            //   message: `Briefing ativado para serviço ${event.serviceId}`
            // });
            
          } catch (error) {
            console.error(`[TriggerSystem] Erro ao notificar equipe:`, error);
          }
        }
      }
    });

    // Regra: Ajustes Aplicados -> Atualizar Tarefas
    registerRule({
      id: 'adjustments_applied_update_tasks',
      eventName: 'adjustments_applied',
      description: 'Atualiza tarefas quando ajustes são aplicados',
      action: async (event) => {
        if (event.serviceId && event.entityId) {
          console.log(`[TriggerSystem] Atualizando tarefas com ajustes ${event.entityId}`);
          ...
          try {
            ...
            // Obter ajustes
            const adjustments = await briefingService.getActiveAdjustmentsByService(event.serviceId);
            
            // Aplicar ajustes às tarefas existentes
            const result = await briefingService.applyAdjustmentsToTasks([], event.serviceId);
            
            console.log(`[TriggerSystem] Tarefas atualizadas: ${result.adjustedTasks.length} ajustadas, ${result.newTasks.length} novas`);
            toast.success(`Tarefas atualizadas: ${result.adjustedTasks.length} ajustadas, ${result.newTasks.length} novas`);
            
          } catch (error) {
            console.error(`[TriggerSystem] Erro ao atualizar tarefas:`, error);
          }
        }
      }
    });
