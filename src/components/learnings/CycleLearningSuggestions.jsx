...
/**
 * Componente para exibir sugestões de aprendizados em ciclos
 */
export default function CycleLearningSuggestions({ 
  cycleId, 
  onLearningApplied,
  showTitle = true,
  limit = 3
  ...
  const loadSuggestions = async () => {
    try {
      const suggestions = await getCycleSuggestions(cycleId, limit);
      ...
      <Card className="border-slate-200 bg-slate-50">
        <CardContent className="p-6">
          <div className="text-center text-slate-600">
            ...
            <p>Nenhuma sugestão disponível para este ciclo.</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            {suggestions.map((learning, index) => (
              ...
              >
                <Card 
                  className="cursor-pointer hover:shadow-md transition-all duration-200 border-blue-100 hover:border-blue-300"
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button
              size="sm"
              onClick={() => getCycleSuggestions(cycleId, 10)}
