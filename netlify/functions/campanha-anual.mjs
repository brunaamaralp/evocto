/**
 * Netlify Function — geração de campanhas do plano anual (9 dims).
 * Exposed as /api/campanha-anual via netlify.toml / _redirects.
 */
...
export async function handler(event) {
  try {
    ...
    const res = buildRes();
    await gerarCampanhaAnualHandler(req, res);
    if (!res.finished) {
      return {
        statusCode: 500,
    ...
    console.error('[netlify campanha-anual]', err);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
