...
/**
 * Portal do Cliente: identidade vem só do profile autenticado.
 * Nunca confiar em clientId enviado pelo frontend.
 */
export async function requireClient(req, res) {
  ...
  const profile = await getProfileForUser(tables, databaseId, user.$id);
  if (!profile) {
    res.status(403).json({ error: 'no_profile', message: 'Perfil não encontrado' });
    return null;
  }
  const role = String(profile.role || '').toLowerCase();
  if (role !== 'client') {
    res.status(403).json({ error: 'forbidden', message: 'Acesso restrito ao portal do cliente' });
    return null;
  }
  ...
  if (!clientId) {
    res.status(403).json({
      error: 'no_client',
      message: 'Conta de cliente sem vínculo. Contate sua agência.',
    });
    return null;
  }
  const agencyId = String(profile.agencyId || '').trim();
  if (!agencyId) {
    res.status(403).json({ error: 'no_agency', message: 'Usuário sem agência' });
    return null;
  }
