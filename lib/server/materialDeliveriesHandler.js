...
import {
  ...
  encryptSecret,
  decryptSecret,
  MaterialCryptoConfigError,
  isMaterialEncryptionConfigured,
...
function cryptoConfigError(res) {
  return json(res, 503, {
    ok: false,
    error: 'crypto_not_configured',
    message: 'Defina MATERIAL_ENCRYPTION_KEY no ambiente do servidor',
  });
}

function requireMaterialEncryption(res) {
  if (isMaterialEncryptionConfigured()) return true;
  cryptoConfigError(res);
  return false;
}
...
async function handleDriveConnect(req, res) {
  ...
  if (!requireMaterialEncryption(res)) return;
  try {
...
async function createApprovalLink(tables, databaseId, {
  ...
  await tables.createRow({
    ...
    permissions: rowTeamPerms(agencyId),
  });
