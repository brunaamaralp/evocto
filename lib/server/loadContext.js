...
import { findAnnualPlan, slicePlanMonth } from './planContext.js';
...
export async function loadContext(empresaId, mesAtual, opts = {}) {
  ...
  let plano_mes = null;
  try {
    const plan = await findAnnualPlan({
      databases,
      empresaId,
      ano: anoAtual,
    });
    plano_mes = slicePlanMonth(plan, mes);
  } catch (err) {
    console.warn('[loadContext] plano_mes:', err?.message || err);
  }
