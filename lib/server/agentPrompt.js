...
### Plano anual (mês atual):
${formatPlanoMes(ctx?.plano_mes)}
  `.trim();
}

function formatPlanoMes(plano) {
  if (!plano) return 'Sem plano anual para este mês.';
  const tema = plano.tema;
  const seed = plano.seed;
  const campanha = plano.campanha;
  const linhas = [
    `Status do mês: ${campanha?.status_mes || 'sem campanha gerada'}`,
    tema?.titulo
      ? `Tema: ${tema.titulo}${tema.ideia_central ? ` — ${tema.ideia_central}` : ''}`
      : 'Tema: —',
    seed?.nome_campanha_sugerido
      ? `Seed: ${seed.nome_campanha_sugerido}`
      : null,
    campanha?.nome_campanha
      ? `Campanha planejada: ${campanha.nome_campanha}`
      : null,
    campanha?.resumo_executivo
      ? `Resumo: ${String(campanha.resumo_executivo).slice(0, 400)}`
      : null,
    campanha?.produto_focal ? `Produto focal: ${campanha.produto_focal}` : null,
  ].filter(Boolean);
  return linhas.join('\n');
}
