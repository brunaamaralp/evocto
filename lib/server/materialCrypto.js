/**
 * Crypto helpers for Material Deliveries (public review tokens, Drive refresh,
 * OAuth state, resumable upload URLs).
 *
 * Requires MATERIAL_ENCRYPTION_KEY — never falls back to APPWRITE_API_KEY.
 */
import { createHash, createHmac, randomBytes, createCipheriv, createDecipheriv } from 'node:crypto';

const TOKEN_BYTES = 32;

export const MATERIAL_ENCRYPTION_KEY_ENV = 'MATERIAL_ENCRYPTION_KEY';

export class MaterialCryptoConfigError extends Error {
  constructor(message = `Defina ${MATERIAL_ENCRYPTION_KEY_ENV} (string forte ou 64 hex)`) {
    super(message);
    this.name = 'MaterialCryptoConfigError';
    this.code = 'crypto_not_configured';
  }
}

export function isMaterialEncryptionConfigured() {
  return Boolean(String(process.env.MATERIAL_ENCRYPTION_KEY || '').trim());
}

function getEncryptionKey() {
  const raw = String(process.env.MATERIAL_ENCRYPTION_KEY || '').trim();
  if (!raw) {
    throw new MaterialCryptoConfigError();
  }
