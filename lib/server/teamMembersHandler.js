...
export default async function teamMembersHandler(req, res) {
  ...
  try {
    ...
    if (code === 409 || msg.includes('already') || msg.includes('exists')) {
      res.status(409).json({
        ...
        error: 'email_in_use',
        message:
          'Já existe uma conta Auth com este e-mail (pode ser de uma tentativa anterior). Use outro e-mail ou remova o usuário no Appwrite Console.',
      });
      return;
    }
  ...
  try {
    ...
    console.error('[team-members] createMembership:', err);
    // Usuário Auth já criado — ainda tentamos gravar o profile
  }
