...
import { decryptSecret } from './materialCrypto.js';
...
/**
 * Materiais aguardando aprovação do cliente (material_deliveries).
 * Soft-fail se a tabela ainda não existir no ambiente.
 */
async function listMaterialApprovals(ctx, req) {
  const { tables, databaseId, clientId, agencyId } = ctx;
  let deliveries = [];
  try {
    deliveries = await listRows(tables, databaseId, 'material_deliveries', [
      Query.equal('agencyId', agencyId),
      Query.equal('clientId', clientId),
      Query.equal('status', 'awaiting_approval'),
      Query.limit(50),
    ]);
  } catch {
    return [];
  ...
  for (const delivery of deliveries) {
    if (delivery.clientId !== clientId || delivery.agencyId !== agencyId) continue;
    if (delivery.archivedAt) continue;

    let version = null;
    if (delivery.currentVersionId) {
      version = await getRowSafe(
        tables,
        databaseId,
        'material_delivery_versions',
        delivery.currentVersionId
      );
    }
    const canDecide =
      delivery.status === 'awaiting_approval' && version?.reviewStatus === 'submitted';

    let reviewUrl = null;
    if (delivery.approvalRequestId) {
      const approval = await getRowSafe(
        tables,
        databaseId,
        'approval_requests',
      ...
      if (
        ...
        approval.agencyId === agencyId &&
        approval.status === 'active' &&
        approval.encryptedToken
      ) {
        try {
          const rawToken = decryptSecret(approval.encryptedToken);
          if (rawToken && origin) {
            reviewUrl = `${origin}/review/${encodeURIComponent(rawToken)}`;
          }
        } catch {
          reviewUrl = null;
        }
    ...
    out.push({
      ...
      kind: 'material',
      title: delivery.title || 'Material para aprovação',
      description: delivery.description || null,
      status: 'pending',
      contentType: 'material_delivery',
      contentId: String(delivery.id),
      serviceId: delivery.serviceId || null,
      expiresAt: null,
      canDecide,
      reviewUrl,
      version: version
...
async function listContentApprovals(ctx) {
  const tasks = await listClientVisibleRows(ctx, 'tasks', 100);
  const contentTasks = tasks.filter(isContentApprovalTask);
  const serviceMap = await loadServiceNameMap(
    ctx.tables,
    ctx.databaseId,
    ctx.agencyId,
    ctx.clientId,
    contentTasks.map((t) => t.serviceId)
  );
  return contentTasks
    .map((t) =>
      toContentApprovalDto(t, {
...
async function listAllPortalApprovals(ctx, req) {
  const [classic, materials, content] = await Promise.all([
    listPendingApprovals(ctx),
    listMaterialApprovals(ctx, req),
    listContentApprovals(ctx),
  ]);
  return [...content, ...materials, ...classic];
}
...
async function handleOverview(ctx, res, req) {
  const [sharedTasks, documents, approvals, mvp, sharedMonths] = await Promise.all([
    listSharedTasks(ctx),
    listVisibleDocuments(ctx),
    listAllPortalApprovals(ctx, req),
    loadMvpOverviewParts(ctx),
    listSharedMonths(ctx),
  ]);
  ...
  return json(res, 200, {
    ...
    stats: {
      ...
      sharedMonths: sharedMonths.length,
    },
    services,
    ...
    sharedMonths: sharedMonths.slice(0, 6),
...
async function handleDecideContent(ctx, res, req) {
  const body = readBody(req);
  const taskId = String(body.taskId || body.approvalId || body.id || '').trim();
  ...
  if (!taskId || !action) {
    return json(res, 400, {
      error: 'invalid',
      message: 'taskId e action (approve|reject) são obrigatórios',
    });
  }
  ...
  const task = await getRowSafe(ctx.tables, ctx.databaseId, 'tasks', taskId);
  if (!task) {
    return json(res, 404, { error: 'not_found', message: 'Conteúdo não encontrado' });
  }
  if (task.clientId !== ctx.clientId || task.agencyId !== ctx.agencyId) {
    return json(res, 403, { error: 'forbidden', message: 'Conteúdo de outro cliente' });
  }
  if (!isClientVisible(task)) {
    return json(res, 403, {
      error: 'forbidden',
      message: 'Este conteúdo não está compartilhado com você',
    });
  }
  if (!isContentApprovalTask(task)) {
    return json(res, 409, {
      error: 'not_pending',
      message: 'Este conteúdo não está aguardando aprovação',
    });
  }

  const now = new Date().toISOString();
  const actorName = ctx.profile.full_name || ctx.profile.name || ctx.user.email || 'Cliente';
  ...
  comments.push({
    ...
    content:
      action === 'approve'
        ? comment
          ? `Aprovado pelo cliente: ${comment}`
          : 'Aprovado pelo cliente'
        : `Ajustes solicitados pelo cliente: ${comment}`,
  ...
  if (action === 'approve') {
    ...
    merged.kanbanColumn = 'completed';
    merged.completedAt = now;
    merged.completedBy = ctx.user.$id;
    merged.completedByRole = 'client';
  } else {
    merged.status = 'in_progress';
    merged.kanbanColumn = 'in_progress';
    merged.clientFeedback = comment;
  }
  ...
  const updated = await getRowSafe(ctx.tables, ctx.databaseId, 'tasks', taskId);
  return json(res, 200, {
    ok: true,
    success: true,
    taskId,
    decision: action === 'approve' ? 'approved' : 'changes_requested',
    status: updated?.status || (action === 'approve' ? 'completed' : 'in_progress'),
  });
}

async function listSharedCycleRows(ctx) {
  const { tables, databaseId, clientId, agencyId } = ctx;
  let rows = [];
  try {
    rows = await listRows(tables, databaseId, 'cycle_plans', [
      Query.equal('agencyId', agencyId),
      Query.equal('clientId', clientId),
      Query.limit(100),
    ]);
...
function taskBelongsToCycle(task, cycleId) {
...
async function listSharedMonths(ctx) {
  const cycles = await listSharedCycleRows(ctx);
  const serviceMap = await loadServiceNameMap(
    ctx.tables,
    ctx.databaseId,
    ctx.agencyId,
    ctx.clientId,
    cycles.map((c) => c.serviceId)
  );
  return sortSharedMonths(
    cycles
      .map((c) =>
        toSharedMonthDto(c, {
          serviceName: serviceMap.get(c.serviceId) || null,
        })
      )
      .filter(Boolean)
  );
...
async function buildSharedMonthDetail(ctx, cycleId) {
  const cycle = await getRowSafe(ctx.tables, ctx.databaseId, 'cycle_plans', cycleId);
  if (!cycle || cycle.clientId !== ctx.clientId || cycle.agencyId !== ctx.agencyId) {
    return null;
  }
  ...
  const month = toSharedMonthDto(cycle, {
    serviceName: serviceMap.get(cycle.serviceId) || null,
  });

  const allVisible = await listClientVisibleRows(ctx, 'tasks', 100);
  const tasksInMonth = allVisible.filter((t) => taskBelongsToCycle(t, cycle.id));

  const items = tasksInMonth
    .map((t) => {
      const approval = toContentApprovalDto(t, {
        serviceName: serviceMap.get(t.serviceId) || null,
      });
      if (approval) return { ...approval, itemKind: 'approval' };
      const dto = toPortalTaskDto(t, {
        serviceName: serviceMap.get(t.serviceId) || serviceMap.get(cycle.serviceId) || null,
      });
      if (!dto) return null;
      return {
        ...dto,
    ...
    .filter(Boolean)
    .sort((a, b) => {
      const aPending = a.itemKind === 'approval' ? 0 : 1;
      const bPending = b.itemKind === 'approval' ? 0 : 1;
...
async function handleSharedMonths(ctx, res) {
  const months = await listSharedMonths(ctx);
  return json(res, 200, { ok: true, months });
}

async function handleSharedMonth(ctx, res, req) {
  const id = String(req.query?.id || req.query?.cyclePlanId || '').trim();
  if (!id) {
    return json(res, 400, { error: 'id_required', message: 'Informe ?id=' });
  }
  const detail = await buildSharedMonthDetail(ctx, id);
  if (!detail) {
    return json(res, 404, {
      error: 'not_found',
      message: 'Mês não encontrado ou ainda não compartilhado',
    });
  }
...
async function handlePendingActions(ctx, res, req) {
...
async function handleApprovals(ctx, res, req) {
  const approvals = await listAllPortalApprovals(ctx, req);
  ...
  try {
    if (req.method === 'GET') {
      switch (route) {
        ...
        case 'overview':
          return await handleOverview(ctx, res, req);
        case 'annual-plan':
        ...
        case 'shared-months':
          return await handleSharedMonths(ctx, res);
        case 'shared-month':
          return await handleSharedMonth(ctx, res, req);
        case 'documents':
          return await handleDocuments(ctx, res);
        case 'services':
          return await handleServices(ctx, res);
        case 'pending-actions':
          return await handlePendingActions(ctx, res, req);
        case 'approvals':
          return await handleApprovals(ctx, res, req);
        ...
        default:
          return json(res, 400, {
            error: 'unknown_route',
            message:
              'Informe ?route=bootstrap|overview|annual-plan|campaigns|campaign|client-actions|complete-client-action|decide-content|shared-tasks|shared-months|shared-month|documents|services|pending-actions|approvals|approval-detail|decide',
          });
      }
    }
    ...
    if (req.method === 'POST' && route === 'decide-content') {
      return await handleDecideContent(ctx, res, req);
    }

    if (req.method === 'POST' && route === 'decide') {
      const body = readBody(req);
      // Conteúdo da tarefa (hub) usa decide-content; mantém alias se kind=content
      if (
        body.kind === 'content' ||
        body.contentType === 'content_task' ||
        String(body.contentType || '') === 'content'
      ) {
        return await handleDecideContent(ctx, res, req);
      }
