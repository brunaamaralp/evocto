...
/** 7 fases Narrativa — PLANEJAMENTO começa finalizado no save-brief */
const NARRATIVA_FASES = [
  { key: 'planejamento', label: 'PLANEJAMENTO' },
  { key: 'roteiros', label: 'ROTEIROS' },
  { key: 'foto_e_video', label: 'FOTO E VÍDEO' },
  { key: 'aprovacao_interna', label: 'APROVAÇÃO INTERNA' },
  { key: 'calendario_cliente', label: 'CALENDÁRIO CLIENTE' },
...
Ciclo esperado: **${String(ciclo).toUpperCase()}**
...
function buildPhaseTasks(startYmd) {
  return NARRATIVA_FASES.map((fase, index) => ({
    id: `fase_${fase.key}`,
    key: fase.key,
    name: fase.label,
    order: index + 1,
    status: index === 0 ? 'concluida' : 'nao_iniciada',
    gate_status: null,
    tasks: [
      {
        id: `${fase.key}_main`,
        titulo: fase.label,
        status: index === 0 ? 'concluida' : 'aberta',
        phase: fase.key,
      },
    ],
  }));
...
async function handleSaveBrief(req, res) {
  ...
  const briefPayload = {
    ...
    ciclo_comercial: dims.ciclo || conversation.ciclo_comercial || contexto?.ciclo_proximo || 'vendas',
  ...
  const phases = buildPhaseTasks(start);
  const cyclePayload = {
    cyclePeriod: `${MES_NOMES[mes] || mes} ${ano}`,
    start_date: start,
    startDate: start,
    end_date: end,
    pipeline: 'narrativa',
    briefId,
    tipo_campanha: briefPayload.tipo_campanha,
    ciclo_comercial: briefPayload.ciclo_comercial,
    linha_focal: briefPayload.linha_focal,
    mes,
    ano,
    agentConversationId: conversationId,
    planData: {
      phases,
      tarefas: phases.flatMap((p) => p.tasks),
    },
    phases,
  };
  ...
  const cycleRow = {
    status: 'approved',
    title: String(`Ciclo ${mes}/${ano} — ${nome}`).slice(0, 255),
    payload: JSON.stringify(cyclePayload),
  };
  if (agencyId) cycleRow.agencyId = agencyId;
  if (clientId) cycleRow.clientId = clientId;

  await databases.createDocument(dbId, CYCLE_PLANS_COL, cycleId, cycleRow);
